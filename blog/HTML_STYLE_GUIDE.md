...
| **Coursera（コーセラ）** | オンライン学習プラットフォーム（世界最大級） | 帯同妻のキャリアブランク対策／帰国転職準備／スキルアップ／就労ビザがない場合でも学べる | ✅A8.net・もしもアフィリエイト経由で提携可。Google・IBM等のプロフェッショナル認定が取得できる。「就労できない帯同ビザでもスキルを磨ける」という切り口で帰国転職・キャリア記事に自然に設置 |
| **Airalo** | 海外eSIM（グローバル最大手） | カナダ渡航準備／移住初期の通信確保／一時帰国 | ✅申込要。公式パートナープログラム（partners.airalo.com）またはアクセストレード経由で提携可。コミッション型。Wiseとセットで紹介すると相乗効果◎ |
| **Holafly（オラフライ）** | 海外eSIM（データ無制限が強み） | カナダ渡航準備／移住初期の通信確保／一時帰国 | ✅申込要。Holafly公式アフィリエイトプログラムに直接申込。読者に割引クーポンコードを提供できる。LINE・日本語サポートが強みで日本人読者に説明しやすい |
...
### 設置のルール
- **【必須】関連アフィリがある記事は、最低2箇所に設置する**：①本文の早い段階に「ソフトな導線」（囲み枠の1文＋リンク）、②まとめ直後に「明確なCTAボタン」（目立つ色付きボックス＋ボタン型リンク）。末尾の小さな注記リンクだけで済ませない（クリックされないため）
- **CTAボタンの型**：`<div>`で背景色付きボックスを作り、ボタン型リンクを中央に置く。見出し（ベネフィット）＋説明1〜2行＋ボタン＋注記の構成。ブランドカラーを使う（例：Wise=グリーン#163300/#9fe870）
- **文脈優先**：記事テーマと関連が薄いリンクは貼らない（信頼が最優先）
- **リンク先URL**：うりの紹介リンクに差し替える前提で公式URLを仮置きし、「※うりの紹介リンクに差し替えてご利用ください」と注記する
...
- **セルフチェック**：記事を書き終えたら「この記事に関連するアフィリが保有リストにあるか？あるなら本文＋CTAの2箇所に入れたか？」を必ず確認してから完成とする
